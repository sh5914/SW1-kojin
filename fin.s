...
TIMER_TICK:
	...
	* LEDは99:59 (5999秒) までしか表示しないため、それを超えたら0にリセット
	cmpi.w #5999, %d0
	ble	CALC_TIME			/* 5999秒以下なら、時刻計算へ */
	move.w #0, SECONDS			/* 6000秒に達したら、カウンタを0にリセット */
	move.w #0, %d0			/* d0 (総秒数) も0にする */
	
CALC_TIME:
	move.w %d0, %d2			/* d2.w = 総秒数 (除算の作業用にコピー) */
	moveq #60, %d1			/* d1 = 60 (除数) */
	
	* divu.w %d1, %d2 は (d2.l / d1.w) を計算する
	* d2(32bit) = 総秒数
	* d1(16bit) = 60
	* 結果: d2.l = [16bit 剰余(秒) | 16bit 商(分)]
	divu.w %d1, %d2
	
	move.w %d2, %d0			/* d0.w = 商 (分) */
	swap %d2			/* d2.l = [16bit 商(分) | 16bit 剰余(秒)] */
					/* これで d2.w に「秒」が入った状態になる */

	* --- 分(MM)を計算して LED7(十の位), LED6(一の位) に表示 ---
	move.w %d0, %d1			/* d1.w = 分 (作業用) */
	divu.w #10, %d1			/* d1.l / 10 => d1.l = [剰余(M1) | 商(M10)] */
	move.b %d1, %d3			/* d3.b = 商 (分の十の位) */
	swap %d1			/* d1.l = [商(M10) | 剰余(M1)] */
	add.b #'0', %d3			/* M10 を ASCII文字に変換 */
	move.b %d3, LED7		/* LED7 に表示 */
	add.b #'0', %d1			/* M1 を ASCII文字に変換 */
	move.b %d1, LED6		/* LED6 に表示 */
	
	move.b #':', LED5		/* LED5 にコロンを表示 */

	* --- 秒(SS)を計算して LED4(十の位), LED3(一の位) に表示 ---
	move.w %d2, %d1			/* d1.w = 秒 (作業用) */
	divu.w #10, %d1			/* d1.l / 10 => d1.l = [剰余(S1) | 商(S10)] */
	move.b %d1, %d3			/* d3.b = 商 (秒の十の位) */
	swap %d1			/* d1.l = [商(S10) | 剰余(S1)] */
	add.b #'0', %d3			/* S10 を ASCII文字に変換 */
	move.b %d3, LED4		/* LED4 に表示 */
	add.b #'0', %d1			/* S1 を ASCII文字に変換 */
	move.b %d1, LED3		/* LED3 に表示 */
