...
MSG_RESULT_CORRECT:
	.ascii "\r\nCorrect: "
.equ MSG_RESULT_CORRECT_LEN, . - MSG_RESULT_CORRECT

MSG_RESULT_ERROR:
	.ascii "\r\nError: "
.equ MSG_RESULT_ERROR_LEN, . - MSG_RESULT_ERROR

MSG_NEWLINE:
	.ascii "\r\n"
.equ MSG_NEWLINE_LEN, . - MSG_NEWLINE
...
ERROR_COUNT:
    .ds.w 1     /* 誤り数 (2バイト) */
NUMBER_BUFFER:
    .ds.b 6     /* 数値 -> 文字列変換用バッファ (16bit最大5桁 + 終端用1) */
...
* 入力: %d0.w (表示したい数値)
* 破壊: %d0, %d1, %d2, %d3, %a0 (PUTSTRING呼び出しのため)
*----------------------------------------------------------------------
PRINT_NUMBER_W:
	...
	* --- 0だった場合の特別処理 ---
	...
	bne     .L_CONV_LOOP
	* 0の場合
	move.b  #'0', (%a0)+          /* '0' をバッファに書き込む */
	move.l  #1, %d1               /* 桁数は 1 */
	bra     .L_CONV_END

	* --- 10進数変換ループ (10で割って余りをスタックに積む) ---
.L_CONV_LOOP:
	move.w  #10, %d3
	divu.w  %d3, %d2              /* d2(32bit) / #10 (16bit) */
								  /* d2.w = 商, d2の上位16bit = 余り */
	swap    %d2                   /* d2.w = 余り, d2の上位16bit = 商 */
	move.w  %d2, -(%sp)           /* 余り(数値)をスタックにプッシュ */
	addq.l  #1, %d1               /* 桁数カウンタ++ */
	
	swap    %d2                   /* d2.w = 商 */
	cmpi.w  #0, %d2               /* 商が 0 か？ */
	bne     .L_CONV_LOOP          /* 0でなければループ続行 */

	* --- スタックから取り出してASCIIとしてバッファに格納 ---
.L_POP_LOOP:
	move.w  (%sp)+, %d2           /* スタックからポップ (数値) */
	add.b   #'0', %d2             /* ASCII文字 '0'～'9' に変換 */
	move.b  %d2, (%a0)+           /* バッファに書き込む */
	
	subq.l  #1, %d1               /* 桁数カウンタ-- */
	bne     .L_POP_LOOP           /* カウンタが0になるまでループ */

.L_CONV_END:
	* --- PUTSTRING でバッファの内容を表示 ---
	* PUTSTRING(ch=0, p=NUMBER_BUFFER, size=桁数(d1))
...
GAME_ENTRY_POINT:
	move.l #0, GAME_LEVEL
  move.w #0, CORRECT_COUNT
	move.w #0, ERROR_COUNT
  move.l #SYSCALL_NUM_PUTSTRING, %D0
	move.l #0, %D1
	move.l #MSG_START, %D2
...
HANDLE_ERROR:
	addi.w #1, ERROR_COUNT      /* 誤りカウントをインクリメント */
	bra WAIT_FOR_INPUT          /* 再度、入力待機に戻る */
...
ALL_FINISHED:
	...
	* --- 正解数を表示 ---
	move.l #SYSCALL_NUM_PUTSTRING, %D0
	move.l #0, %D1
	move.l #MSG_RESULT_CORRECT, %D2
	move.l #MSG_RESULT_CORRECT_LEN, %D3
	trap #0
	
	move.w CORRECT_COUNT, %d0   /* %d0 に正解数をセット */
	jsr PRINT_NUMBER_W          /* ★作成した数値表示ルーチン呼び出し */

	* --- 誤り数を表示 ---
	move.l #SYSCALL_NUM_PUTSTRING, %D0
	move.l #0, %D1
	move.l #MSG_RESULT_ERROR, %D2
	move.l #MSG_RESULT_ERROR_LEN, %D3
	trap #0

	move.w ERROR_COUNT, %d0     /* %d0 に誤り数をセット */
	jsr PRINT_NUMBER_W          /* ★作成した数値表示ルーチン呼び出し */

	* --- 最後の改行 ---
	move.l #SYSCALL_NUM_PUTSTRING, %D0
	move.l #0, %D1
	move.l #MSG_NEWLINE, %D2
	move.l #MSG_NEWLINE_LEN, %D3
	trap #0
	* --- ★追加ここまで ---
