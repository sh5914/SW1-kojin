...
* 構造: [文字列ポインタ(4B), 文字列長(4B), 制限時間(4B)] (計12B)
STAGE_TABLE:
	.dc.l	TARGET_STRING_1, TARGET_LEN_1, 4
	.dc.l	TARGET_STRING_2, TARGET_LEN_2, 4
	.dc.l	TARGET_STRING_3, TARGET_LEN_3, 8
	.dc.l	TARGET_STRING_4, TARGET_LEN_4, 16
...
RANDOM_SEED:
	.ds.w 1		/* 乱数のタネ (16bit) */
STAGE_ORDER:
	.ds.w 4		/* ステージの順番を格納する配列 (例: [2, 0, 3, 1]) */
...
.L_TIMER_EXIT:
  ...
  *----------------------------------------------------------------------
* GET_RANDOM_W: 16bitの擬似乱数を生成 (LCG法)
* 出力: %d0.w (16bit乱数)
* 破壊: %d0, %d1
*----------------------------------------------------------------------
GET_RANDOM_W:
	...
	move.w	#25173, %d1
	mulu.w	%d1, %d0			/* seed = (seed * 25173) ... */
	add.w	#13849, %d0			/* ... + 13849 (mod 65536) */
...
.L_SHUFFLE_LOOP:
	* --- 乱数 j を生成 (j = rand % (i+1)) ---
	jsr		GET_RANDOM_W		/* d0.w = 16bit乱数 */
	move.l	%d0, %d2
	swap	%d2
	clr.w	%d2
	swap	%d2					/* d2 = [0, 乱数] (上位ビットをクリア) */
	move.w	%d1, %d3			/* d3 = i */
	addq.w	#1, %d3				/* d3 = i + 1 */
	divu.w	%d3, %d2			/* d2 = (乱数 / (i+1)) */
	swap	%d2					/* d2.w = 剰余 (j) */
...
GAME_ENTRY_POINT:
	move.l #0, GAME_LEVEL
  move.w	TCN1, RANDOM_SEED		/* タイマカウンタを乱数のタネに設定 */
	lea.l	STAGE_ORDER, %a0		/* STAGE_ORDER を [0, 1, 2, 3] に初期化 */
	move.w	#0, 0(%a0)
	move.w	#1, 2(%a0)
	move.w	#2, 4(%a0)
	move.w	#3, 6(%a0)
	jsr		SHUFFLE_STAGES			/* ステージ配列をシャッフル */
...
TYPING_GAME_LOOP:
	...
	* --- d1.w (インデックス) * 12 (テーブルのエントリ長) を計算 ---
	move.w	%d1, %d0
	move.w	#12, %d2
	mulu.w	%d2, %d0			/* d0 = インデックス * 12 (オフセット) */
	
	* --- テーブルからデータを取得 ---
	lea.l	STAGE_TABLE, %a1
	move.l	(%a1,%d0.l), %a2		/* %a2 = 文字列ポインタ */
	move.l	4(%a1,%d0.l), %d4	/* %d4 = 文字列長 */
	move.l	8(%a1,%d0.l), %d0	/* %d0 = 制限時間 */

	* --- この %a2, %d4, %d0 を使ってステージを初期化 ---
