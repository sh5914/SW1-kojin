...
******************************
*----------------------------------------------------------------------
* 1秒タイマーコールバック (LED表示 AND 残り時間カウントダウン)
*----------------------------------------------------------------------
TIMER_TICK:
...
CALC_TIME:
	...
	* --- 2. (★追加) ゲームの残り時間をカウントダウン ---
	move.l TIME_REMAINING, %d0
	cmpi.l #0, %d0				/* 残り時間は 0 か？ */
	beq .L_TIMER_TICK_EXIT		/* 既に 0 なら何もしない */
	
	subq.l #1, %d0				/* 残り時間を 1 秒減らす */
	move.l %d0, TIME_REMAINING
	
	cmpi.l #0, %d0				/* 0 になったか？ */
	bne .L_TIMER_TICK_EXIT		/* 0 でなければ終了 */
	
	move.b #1, GAME_OVER_FLAG	/* 0 になったら「ゲームオーバーフラグ」を 1 にする */
...
MSG_STATS_CORRECT:
	.ascii "\r\nCorrect: "
.equ MSG_STATS_CORRECT_LEN, . - MSG_STATS_CORRECT

MSG_STATS_ERROR:
	.ascii "\r\nError: "
.equ MSG_STATS_ERROR_LEN, . - MSG_STATS_ERROR

MSG_STATS_SPEED:
	.ascii "\r\nSpeed (SPM): "
.equ MSG_STATS_SPEED_LEN, . - MSG_STATS_SPEED
...
TOTAL_CORRECT:
	.ds.l 1				/* 合計正解タイプ数 (32-bit) */
TOTAL_ERROR:
	.ds.l 1				/* 合計エラータイプ数 (32-bit) */
...
*----------------------------------------------------------------------
RESET_TIMER:
	movem.l	%d0/%a0,-(%sp)		/* ★修正: D0/A0レジスタをスタックに退避 */
	
	move.l	 #TCTL1, %A0	 /* A0 に TCTL1 のアドレスをロード */
	andi.w	 #0xFFFE, (%A0)	/* タイマー停止 (TCTL1 Bit 0 'TEN' = 0) */
* --- ★ここから修正 (ori.l (%a0) は M68000 に存在しないため) ---
	move.l	 #IMR, %A0	 /* A0 に IMR のアドレスをロード */
	move.l	 (%A0), %D0	/* (1) IMRの現在の値を D0 に読み込む (Read) */
	ori.l	#0x00000002, %D0	/* (2) D0レジスタ内でBit 1を立てる (Modify) */
	move.l	%D0, (%A0)	/* (3) D0の値をIMRに書き戻す (Write) */
* --- ★ここまで修正 ---

	move.w	#0x0000, TSTAT1	/* 保留中の割り込みフラグをクリア */

	movem.l	(%sp)+,%d0/%a0		/* ★修正: D0/A0レジスタをスタックから復帰 */
	rts
...
* --- ★ここから修正 (andi.l (%a0) は M68000 に存在しないため) ---
	move.l	 #IMR, %A0	/* A0 に IMR のアドレスをロード */
	move.l	 (%A0), %D0	/* (1) IMRの現在の値を D0 に読み込む (Read) */
	andi.l	 #0xFFFFFFFD, %D0	/* (2) D0レジスタ内でBit 1をクリア (Modify) */
	move.l	 %D0, (%A0)	/* (3) D0の値をIMRに書き戻す (Write) */
* --- ★ここまで修正 ---
...
* (★新規追加) 10進数表示ルーチン
*
* 役割: %d0.l (32-bit) に入っている数値を10進数の文字列として表示する
*----------------------------------------------------------------------
PRINT_DECIMAL:
	movem.l %d0-%d3, -(%sp)		/* d0-d3 を退避 (d0は digit count に使う) */
	move.l %d0, %d1				/* d1 = 印刷したい数値 (作業用) */
	move.l #10, %d2				/* d2 = 10 (除数) */
	moveq #0, %d0				/* d0 = 桁数カウンター */
...
*	move.b (%sp)+, PRINT_CHAR	/* ★バスエラーの原因 */
	...
	movem.l (%sp)+, %d0/%d2		/* ループ用レジスタを復帰 */
	
	subq.l #1, %d0				/* 桁数カウンターを -1 */
...
*----------------------------------------------------------------------
SHOW_STATS:
	movem.l %d0-%d3, -(%sp)		/* 全レジスタを退避 */

	* 1. 正解数を表示
	move.l #SYSCALL_NUM_PUTSTRING, %d0
	move.l #0, %d1
	move.l #MSG_STATS_CORRECT, %d2
	move.l #MSG_STATS_CORRECT_LEN, %d3
	trap #0
	
	move.l TOTAL_CORRECT, %d0
	jsr PRINT_DECIMAL
	
	* 2. 誤り数を表示
	move.l #SYSCALL_NUM_PUTSTRING, %d0
	move.l #0, %d1
	move.l #MSG_STATS_ERROR, %d2
	move.l #MSG_STATS_ERROR_LEN, %d3
	trap #0
...
WAIT_FOR_INPUT:
	...
	* --- 4. 入力文字をチェック ---
	...
	move.b (%a2, %d1.l), %d6
	cmp.b %d5, %d6
	beq .L_CORRECT_KEY			/* ★変更: 正解なら .L_CORRECT_KEY へ */
	
	addq.l #1, TOTAL_ERROR		/* ★追加: 誤りカウンターを +1 */
	bra WAIT_FOR_INPUT			/* 入力待ちに戻る */
	
.L_CORRECT_KEY:
	* --- 5. 正解した場合 ---
	addq.l #1, TOTAL_CORRECT	/* ★追加: 正解カウンターを +1 */
